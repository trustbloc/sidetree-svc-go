...
import (
	...
	internaljws "github.com/trustbloc/sidetree-go/pkg/jwsutil"
...
func TestOperationHandler_PrepareTxnFiles(t *testing.T) {
	...
	t.Run("success - stale operations not included", func(t *testing.T) {
		...
		op, err := generateQueueOperationWithAnchorTimes(coreoperation.TypeRecover, "stale-recover", 0, expiry)
		require.NoError(t, err)

		ops = append(ops, op)

		// generate stale recover operation
		op, err = generateQueueOperationWithAnchorTimes(coreoperation.TypeDeactivate, "stale-deactivate", 0, expiry)
		require.NoError(t, err)

		ops = append(ops, op)

		// generate stale recover operation
		op, err = generateQueueOperationWithAnchorTimes(coreoperation.TypeUpdate, "stale-update", 0, expiry)
		require.NoError(t, err)
...
func getTestOperations(createOpsNum, updateOpsNum, deactivateOpsNum, recoverOpsNum int) []*operation.QueuedOperation {
	...
	ops = append(ops, generateOperations(createOpsNum, coreoperation.TypeCreate)...)
	ops = append(ops, generateOperations(recoverOpsNum, coreoperation.TypeRecover)...)
	ops = append(ops, generateOperations(deactivateOpsNum, coreoperation.TypeDeactivate)...)
	ops = append(ops, generateOperations(updateOpsNum, coreoperation.TypeUpdate)...)

	return ops
}

func generateOperations(numOfOperations int, opType coreoperation.Type) (ops []*operation.QueuedOperation) {
...
func generateOperationInfo(num int, opType coreoperation.Type) (*operation.QueuedOperation, error) {
...
func generateOperation(num int, opType coreoperation.Type) (*model.Operation, error) {
	op, err := generateOperationBuffer(num, opType)
	if err != nil {
		return nil, err
	}
...
func generateOperationBuffer(num int, opType coreoperation.Type) ([]byte, error) {
	switch opType {
	case coreoperation.TypeCreate:
		return generateCreateOperation(num)
	case coreoperation.TypeRecover:
		return generateRecoverOperation(num)
	case coreoperation.TypeDeactivate:
		return generateDeactivateOperation(num)
	case coreoperation.TypeUpdate:
...
func generateQueueOperationWithAnchorTimes(opType coreoperation.Type, suffix string, from, until int64) (*operation.QueuedOperation, error) {
	var opBuffer []byte
	var err error
