...
func TestHandler_GetTxnOperations(t *testing.T) {
	...
	t.Run("success - deactivate only", func(t *testing.T) {
		...
		require.Equal(t, anchoringInfo.OperationReferences[0].Type, coreoperation.TypeDeactivate)
	...
	t.Run("success - update only", func(t *testing.T) {
		...
		require.Equal(t, anchoringInfo.OperationReferences[0].Type, coreoperation.TypeUpdate)
	...
	t.Run("success - create only", func(t *testing.T) {
		...
		require.Equal(t, anchoringInfo.OperationReferences[0].Type, coreoperation.TypeCreate)
	...
	t.Run("success - recover only", func(t *testing.T) {
		...
		require.Equal(t, anchoringInfo.OperationReferences[0].Type, coreoperation.TypeRecover)
...
func TestHandler_GetBatchFiles(t *testing.T) {
	...
	updateOp, err := generateOperation(1, coreoperation.TypeUpdate)
	require.NoError(t, err)

	recoverOp, err := generateOperation(2, coreoperation.TypeRecover)
	require.NoError(t, err)
	...
	t.Run("error - validate batch counts", func(t *testing.T) {
		...
		recoverOp, err := generateOperation(3, coreoperation.TypeRecover)
		require.NoError(t, err)
...
func TestHandler_assembleBatchOperations(t *testing.T) {
	...
	t.Run("error - core/provisional index, chunk file operation number mismatch", func(t *testing.T) {
		...
		createOp, err := generateOperation(1, coreoperation.TypeCreate)
		require.NoError(t, err)

		updateOp, err := generateOperation(2, coreoperation.TypeUpdate)
		require.NoError(t, err)

		deactivateOp, err := generateOperation(3, coreoperation.TypeDeactivate)
		require.NoError(t, err)
...
func generateDefaultBatchFiles() (*batchFiles, error) {
	...
	deactivateOp, err := generateOperation(4, coreoperation.TypeDeactivate)
	if err != nil {
		return nil, err
	}
