...
import (
	...
	stmocks "github.com/trustbloc/sidetree-go/pkg/mocks"
	...
	"github.com/trustbloc/sidetree-go/pkg/versions/1_0/doctransformer/doctransformer"
	...
	docmocks "github.com/trustbloc/sidetree-svc-go/pkg/dochandler/mocks"
...
func TestDocumentHandler_DefaultDecorator(t *testing.T) {
	t.Run("success - create", func(t *testing.T) {
		...
		updateOp := &coreoperation.Operation{
			Type:         coreoperation.TypeCreate,
			UniqueSuffix: "suffix",
		}
	...
	t.Run("success - update", func(t *testing.T) {
		...
		err = store.Put(&coreoperation.AnchoredOperation{UniqueSuffix: createOp.UniqueSuffix, Type: coreoperation.TypeCreate, OperationRequest: createOpBuffer})
		require.NoError(t, err)
	...
	t.Run("error - document has been deactivated, no further operations allowed", func(t *testing.T) {
		rm := &coreprotocol.ResolutionModel{
			Deactivated: true,
		}
...
func TestDocumentHandler_ProcessOperation_Update(t *testing.T) {
	...
	t.Run("success - unpublished operation store option", func(t *testing.T) {
		...
		opt := WithUnpublishedOperationStore(&mockUnpublishedOpsStore{}, []coreoperation.Type{coreoperation.TypeUpdate})
	...
	t.Run("success - unpublished operation store option(create and update)", func(t *testing.T) {
		...
		unpublishedOperationStore := &mockUnpublishedOpsStore{
			Ops: []*coreoperation.AnchoredOperation{
				{
					Type:             "create",
		...
		dochandler, cleanup := New(namespace, []string{alias}, protocol, writer, processor, &mocks.MetricsProvider{},
			WithUnpublishedOperationStore(unpublishedOperationStore, []coreoperation.Type{coreoperation.TypeCreate, coreoperation.TypeUpdate})), func() { writer.Stop() }
		require.NotNil(t, dochandler)
		defer cleanup()
	...
	t.Run("error - batch writer error (unpublished operation store option)", func(t *testing.T) {
		...
		opt := WithUnpublishedOperationStore(&noopUnpublishedOpsStore{}, []coreoperation.Type{coreoperation.TypeUpdate})
	...
	t.Run("error - unpublished operation store put error", func(t *testing.T) {
		...
		opt := WithUnpublishedOperationStore(
			...
			[]coreoperation.Type{coreoperation.TypeUpdate})
	...
	t.Run("error - unpublished operation store delete error", func(t *testing.T) {
		...
		dochandler.deleteOperationFromUnpublishedOpsStore(&coreoperation.AnchoredOperation{UniqueSuffix: "suffix"})
	})
...
func TestDocumentHandler_ResolveDocument_InitialValue(t *testing.T) {
	...
	t.Run("error - transform create with initial state to external document", func(t *testing.T) {
		transformer := &stmocks.DocumentTransformer{}
...
func getCreateOperationWithInitialState(suffixData *model.SuffixDataModel, delta *model.DeltaModel) (*model.Operation, error) {
	request := &model.CreateRequest{
		Operation:  coreoperation.TypeCreate,
...
func getAnchoredCreateOperation() *coreoperation.AnchoredOperation {
	op := getCreateOperation()

	return getAnchoredOperation(op)
}

func getAnchoredOperation(op *model.Operation) *coreoperation.AnchoredOperation {
...
func getUpdateOperation() *coreoperation.Operation {
	request := &model.UpdateRequest{
		Operation: coreoperation.TypeUpdate,
...
func (m *mockUnpublishedOpsStore) Put(_ *coreoperation.AnchoredOperation) error {
	return m.PutErr
}

func (m *mockUnpublishedOpsStore) Delete(_ *coreoperation.AnchoredOperation) error {
	return m.DeleteErr
}

func (m *mockUnpublishedOpsStore) Get(uniqueSuffix string) ([]*coreoperation.AnchoredOperation, error) {
	if m.GetErr != nil {
		return nil, m.GetErr
	}
...
func (m *mockOperationDecorator) Decorate(op *coreoperation.Operation) (*coreoperation.Operation, error) {
	if m.Err == nil {
		return nil, m.Err
	}
